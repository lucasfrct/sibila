...
class DocumentInfo:
    def __init__(self, path: str = "", name: str = "", size: int = 0, pages: int = 1, mimetype: str = "pdf"):  # noqa: E501
        self.id = 0
        self.path = path
        self.name = name
        self.size = size
    ...
    def dict(self):
        return {'id': self.id, 'path': self.path, 'name': self.name, 'size': self.size, 'pages': self.pages, 'mimetype': self.mimetype}  # noqa: E501
...
class DocumentMetadata:
    ...
    def from_tuple(self, meta_tuple):
        uuid, path, page, name, source, letters, content, distance, line, size, lines, pages, chunk, chunks, mimetype, paragraph, paragraphs = meta_tuple   # noqa: E501
    ...
    def to_model(self):
        return (self.uuid, self.path, self.page, self.name, self.source, self.letters, self.content, self.size, self.lines, self.pages, self.chunks, self.mimetype, self.paragraphs)  # noqa: E501

    # transforma a tupla da base de dados SQL na classe
    def from_model(self, model_tuple):
        uuid, path, page, name, source, letters, content, size, lines, pages, chunks, mimetype, paragraphs = model_tuple  # noqa: E501
...
def lines_with_details(path: str = "", init: int = 1, final: int = 0) -> List[object]:  # noqa: E501
    try:
        ...
        for paragraph in paragraphs:
            ...
            for i, content in enumerate(lns):
                ...
                line = {
                    ...
                    'letters': len(content),
                    'uuid': str(uuid.uuid4()),
                    'source': f"{paragraph['name']}, pg. {paragraph['page']}, ln {i+1}",  # noqa: E501
