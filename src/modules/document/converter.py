...
start_article_pattern = re.compile(
    r'Art\. [0-9]+[A-Za-z]?\s*–?\s*([^\n]*)', re.IGNORECASE)
stop_article_pattern = re.compile(
    r'\b[I|V|X|L|C]+\b|Parágrafo|Art\. [0-9]|\.\.{10,}', re.IGNORECASE)
...
def extract_and_structure_text(lines):
    ...
    for line in lines:
        ...
        data.append([klass, line, current_title, current_chapter, current_section,
                    current_subsection, current_article, current_paragraph, current_incise, current_aline])
