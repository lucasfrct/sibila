...
def matrix_tsne(embeddings):
	...
	tsne = TSNE(n_components=2, perplexity=15, random_state=42, init='random', learning_rate=200)  # ou n_components=3 para 3D
...
def display(embeddings, chunks):
	...
	g = Network(height="850px", width="100%", bgcolor="#222222", font_color="white", notebook=True, cdn_resources="remote", directed=True)
	...
	q = len(embeddings_small) / len(chuncks_small)
	...
	for embedding, chunk in zip(embeddings_small, chuncks_small):
		...
		g.add_node(num_id, label=f"({key}) {chunk}", value=val, x=int(emb_l), y=int(emb_r))
